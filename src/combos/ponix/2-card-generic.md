# Ponix + any Fire King

Adding a second card to the 1-card Ponix combo makes it much more powerful, allowing you to add multiple layers of interruption alongside the board clear that [_Garunix Eternity, Hyang of the Fire Kings_] offers.

Here is a generic combo that works with any Fire King card - it sets up the FK engine and ends on a [_Promethean Princess, Bestower of Flames_] backed up by [_Fire King Avatar Arvata_].

<center>
<video controls>
  <source src="2-card-generic.mp4" type="video/mp4">
  Your browser does not support the MP4 format, or the &lt;video&gt; tag.
</video>
</center>

<div class="warning">

This route stops [_Nibiru, the Primal Being_], as Arvata is the 5th summon.

Under [_Mulcharmy Fuwalos_], your opponent draws 1 card.

</div>

#### Steps
- Normal Summon [_Legendary Fire King Ponix_].
- Activate Ponix - add [_Fire King Sanctuary_] from Deck.
- Activate Sanctuary - place [_Fire King Island_] in Field Zone.
- Activate Island - destroy a Fire King in hand, add [_Sacred Fire King Garunix_] from Deck.
- Activate Sacred in hand - Special Summon it to the field.
- Activate Sacred on field - destroy [_Fire King Avatar Arvata_] from Deck.
- Activate Arvata in GY - Special Summon a Fire King from GY.
- Link Summon [_Promethean Princess, Bestower of Flames_].
- Activate Princess - Special Summon Arvata from GY.

> Depending on the second card there might be other lines available that offer you certain advantages. This line is great when you're first learning the deck, but we encourage reading and learning the other combos as well in order to make the most out of your deck.
>
> Additionally, this line theoretically works with any FIRE Beast, Beast-Warrior or Winged-Beast monster, enabling you to perform it with non-archetypal cards too, should you choose to play a variant of the deck instead of the pure version.

{{#include ../../links.md}}